...
data <- data |>
    ...
    mutate(inflation1 = 100*(deflator_index/lag(deflator_index, n = 4) - 1)) |>
    ungroup()

data |>
    group_by(time) |>
    summarise(mean_inflation = mean(inflation1, na.rm = TRUE)) |>
    ggplot(aes(time, mean_inflation)) +
    geom_line()

## Unit labor cost, unit profit, and unit taxes (income components per product)
data = data |>
    mutate(unit_labor_cost = labor_compensation/chain_linked_valume_index,
...
data = data |>
    group_by(reference_area) |>
    arrange(time, .by_group = TRUE) |>
    mutate(delta_unit_labor_cost = 100*(unit_labor_cost/lag(unit_labor_cost, n = 4) - 1),
...
data |>
    ...
    group_by(time) |>
    summarise(mean_inflation1 = mean(inflation1, na.rm = TRUE),
...
data_long = data |>
    select(c(reference_area, ref_area, time, inflation1, starts_with("contr_"))) |>
    pivot_longer(cols = 4:8, names_to = "series", values_to = "value")

comps = c("contr_unit_labor_cost", "contr_unit_profit", "contr_unit_tax")

data_long |>
    filter(!reference_area %in% country_groups) |>
    filter(series %in% comps) |>
